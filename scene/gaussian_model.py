...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        self._mask = nn.Parameter(torch.zeros((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
            {'params': [self._mask], 'lr': training_args.feature_lr, "name": "mask"},
            {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"}
    ...
    def save_ply(self, path):
        ...
        mask_val = self._mask.detach().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, mask_val, scale, rotation), axis=1)
    ...
    def load_ply(self, path):
        ...
        # Initialize mask properly from PLY if it exists, else zeros
    ...
    def prune_points(self, mask):
        ...
        elif hasattr(self, '_mask') and self._mask is not None:
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_mask=None):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_deformation_table, new_mask=new_mask)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_mask=new_mask)
    ...
    def add_point_by_mask(self, selected_pts_mask, perturb=0):
        ...
        new_mask = self._mask[selected_pts_mask][mask]
